...
This files defines a finite state machine for controlling the robot.  
...
\begin{DoxyCompactList}\small\item\em Function impemented to get the user\textquotesingle{}s command. \end{DoxyCompactList}\item 
...
client\+\_\+rp\+: to ask a random goal at custom service Random\+Position goal\+\_\+position\+: in order to set the random goal generated as a Position\+Goal and send it to /go\+\_\+to\+\_\+point node service\+: read the command sent by the user from the Command service custom message

After initializations it generates a request for the random position in interval \mbox{[}-\/5, 5\mbox{]}, then executes an infinite loop in wich it gets the ranodom position and sends it as a goal position to the /go\+\_\+to\+\_\+point node and waits untill the target is reached. In case in wich the command from /user\+\_\+interface is \char`\"{}stop\char`\"{} (so start==false) the goal is cancelled and the robot is stopped, otherwise the loop is executed again. \mbox{\Hypertarget{state__machine_8cpp_a1a9543636935547580c0657f4c7c0c2b}\label{state__machine_8cpp_a1a9543636935547580c0657f4c7c0c2b}} 
...
Function impemented to get the user\textquotesingle{}s command. 

bool user\+\_\+interface(req, res)


\begin{DoxyParams}{Parameters}
{\em req} & request done by the client (start/stop the robot)\\
\hline
